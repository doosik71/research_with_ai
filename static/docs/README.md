...
문서 작업용 기준 위치는 저장소 루트가 아니라 `static/docs` 폴더이다.
아래에 언급하는 연구 분야 폴더와 안내 문서는 모두 `static/docs` 폴더 아래에 있다.
...
`static/docs` 폴더 아래에서 각 연구 분야는 별도 폴더로 관리한다.
...
1. `static/docs` 폴더에서 [TOPIC.md](TOPIC.md)와 안내 문서를 먼저 확인한다.
