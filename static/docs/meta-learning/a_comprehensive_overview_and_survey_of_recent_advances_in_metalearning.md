...
$$
\mathcal{T}={p(\boldsymbol{x}), p(y|\boldsymbol{x}), \mathcal{L}}
$$

여기서 $\mathcal{L}$은 loss function이고, $p(\boldsymbol{x})$와 $p(y|\boldsymbol{x})$는 입력과 라벨의 생성 분포이다. 또한 task는 상위 task distribution에서 샘플된다고 둔다.
...
$$
P(y|\boldsymbol{x})=
\frac{\exp{\mathbb{E}_{S}[\phi(s_y)a(\boldsymbol{x})]}}
{\sum_{k\in\mathcal{C}}\exp{\mathbb{E}_{S}[\phi(s_k)a(\boldsymbol{x})]}}
$$
...
$$
r_{i,j}=g_{\theta}[f_{\phi}(\boldsymbol{x}_i),f_{\phi}(\boldsymbol{x}^{*}_j)]
$$

그리고 학습은 이 relation score가 같은 클래스면 1, 다른 클래스면 0에 가깝도록 squared loss를 최소화한다.

$$
\min_{\phi,\theta}\sum_{i=1}^{n}\sum_{j=1}^{m}
{r_{i,j}-\mathbb{I}(y_i=y^{*}_j)}^{2}
$$
...
$$
c_k=\frac{1}{|S_k|}\sum_{(\boldsymbol{x}_i,y_i)\in S_k} f*\phi(\boldsymbol{x}_i)
$$

그 뒤 query는 각 class centroid와의 거리로 분류된다.

$$
p_{\phi}(y=k|\boldsymbol{x})=
\frac{\exp(-g[f_{\phi}(\boldsymbol{x}),c_k])}
{\sum_{k'}\exp(-g[f_{\phi}(\boldsymbol{x}),c_{k'}])}
$$
...
$$
p_{\phi,\alpha}(y=k|\boldsymbol{x})=\text{Softmax}(-\alpha g[f_\phi(\boldsymbol{x}),c_k])
$$
...
\sum_{\mathcal{T}_i\sim p(\mathcal{T})}\mathcal{L}_{\mathcal{T}_i}(h_{\phi_i})
$$
...
$$
W_{ij}=\exp\left\{-\frac{g[f_{\phi}(\boldsymbol{x}_i),f_{\phi}(\boldsymbol{x}_j)]}{2\sigma^2}\right\}
$$
...
$$
p(\boldsymbol{x}|\theta)=\prod_{j=1}^{J}
\left\{\int p(\boldsymbol{x}_j|\phi_j)p(\phi_j|\theta)d\phi_j\right\}
$$
...
$$
g(\theta)=n^{-1}\sum_{j=1}^{n}
\left\{
k(\theta_j^l,\theta)\nabla_{\theta_j^l}\log p(\theta_j^l)
+\nabla_{\theta_j^l}k(\theta_j^l,\theta)
\right\}
$$
