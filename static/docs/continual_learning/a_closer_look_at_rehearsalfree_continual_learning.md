...
$$
\mathcal{L}_{ewc}=\sum_{j=1}^{N_{params}}F_{n-1}^{jj}\left(\theta_n^j-\theta_{n-1}^j\right)^2
$$
...
$$
\mathcal{L}_{L2} \propto \sum_j \left(\theta_n^j-\theta_{n-1}^j\right)^2
$$
...
$$
\mathcal{L}_{PredKD}=CE\left(p_{\theta_{n,1:n-1}}(x),, p_{\theta_{n-1,1:n-1}}(x)\right)
$$
...
$$
\mathcal{L}_{FeatKD}= \left|\theta_n^l(x)-\theta_{n-1}^l(x)\right|_2^2
$$
...
$$
A_{i,n}=\frac{1}{|\mathcal{D}_{n}^{test}|}\sum_{(x,y)\in\mathcal{D}_{n}^{test}} \mathbf{1}(\hat{y}(x,\theta_{i,n})=y \mid \hat{y}\in\mathcal{T}_n)
$$
...
$$
R_{i,n}=\frac{1}{|\mathcal{D}_{n}^{test}|}\sum_{(x,y)\in\mathcal{D}_{n}^{test}} \mathbf{1}(\hat{y}(x,\theta_{i,1:n})=y)
$$
