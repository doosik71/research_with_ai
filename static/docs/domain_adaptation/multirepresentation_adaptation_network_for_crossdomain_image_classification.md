...
$$
\hat{d}_{\mathcal{H}}(\mathbf{X}_s,\mathbf{X}_t) = \left| \frac{1}{n_s}\sum_{\mathbf{x}*i \in \mathcal{D}*{\mathbf{X}^s}}\phi(\mathbf{x}_i) - \frac{1}{n_t}\sum_{\mathbf{x}_j \in \mathcal{D}_{\mathbf{X}^t}}\phi(\mathbf{x}_j) \right|^2_{\mathcal{H}}.
$$
