...
$
\mathcal{L}_{src}(f_s;\mathcal{X}_s,\mathcal{Y}_s) = -\mathbb{E}_{(x_s,y_s)} \sum_{k=1}^{K} q_k \log \delta_k(f_s(x_s))
$
...
$
\mathcal{L}_{src}^{ls}(f_s;\mathcal{X}_s,\mathcal{Y}_s) = -\mathbb{E}_{(x_s,y_s)} \sum_{k=1}^{K} q_k^{ls} \log \delta_k(f_s(x_s))
$
